...
| `references/mfc-carchive-serialization.md` | Reversing an MFC (`CObject`/`CArchive`/`IMPLEMENT_SERIAL`) binary's custom serialization format -- the `CRuntimeClass` struct layout, a static xref sweep (`AfxClassInit`) that enumerates every serializable class in one pass, and the exact on-disk class-tag wire format so you can recognize it directly in a real file's bytes. |
